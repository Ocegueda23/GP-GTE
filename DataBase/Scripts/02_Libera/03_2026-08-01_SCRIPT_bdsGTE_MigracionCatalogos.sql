...
/* =====================================================================
   ...
   Descripcion: Migracion B3, bloque Catalogos: Equipos (con lider real,
                ...
                CONFIGURACION DE ENTORNO: el nombre de la base origen se
                ajusta en UN SOLO LUGAR, la variable @BaseOrigen de abajo
                (default 'bdsApollo', el nombre real restaurado). El
                acceso cross-DB se hace via sinonimos temporales
                (creados al inicio, eliminados antes del COMMIT) para no
                repetir el nombre de la base en cada referencia de 3
                partes.
   ===================================================================== */
BEGIN TRY
    ...
    IF EXISTS (SELECT 1 FROM sys.synonyms WHERE name = N'OrigenComplejidad') DROP SYNONYM dbo.OrigenComplejidad;
    SET @SqlSinonimo = N'CREATE SYNONYM dbo.OrigenComplejidad FOR ' + QUOTENAME(@BaseOrigen) + N'.dbo.tblComplejidad';
    EXEC sp_executesql @SqlSinonimo;

    IF EXISTS (SELECT 1 FROM sys.synonyms WHERE name = N'OrigenProyecto') DROP SYNONYM dbo.OrigenProyecto;
    SET @SqlSinonimo = N'CREATE SYNONYM dbo.OrigenProyecto FOR ' + QUOTENAME(@BaseOrigen) + N'.dbo.tblProyecto';
    EXEC sp_executesql @SqlSinonimo;
    ...
    DROP SYNONYM dbo.OrigenComplejidad;
    DROP SYNONYM dbo.OrigenProyecto;
