...
/* =====================================================================
   ...
   Descripcion: Migracion B3, bloque Solicitudes. Origen: bdsApollo.tblEDM
                ...
                CONFIGURACION DE ENTORNO: igual que el script 03, el
                nombre de la base origen se ajusta en la variable
                @BaseOrigen de abajo (default 'bdsApollo'), via un
                sinonimo temporal.
   ===================================================================== */
BEGIN TRY
    ...
    IF EXISTS (SELECT 1 FROM sys.synonyms WHERE name = N'OrigenEDM') DROP SYNONYM dbo.OrigenEDM;
    SET @SqlSinonimo = N'CREATE SYNONYM dbo.OrigenEDM FOR ' + QUOTENAME(@BaseOrigen) + N'.dbo.tblEDM';
