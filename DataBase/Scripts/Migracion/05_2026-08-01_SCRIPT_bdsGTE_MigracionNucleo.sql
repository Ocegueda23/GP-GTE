...
/* =====================================================================
   ...
   Descripcion: Migracion B3, bloque Nucleo (el bloque grande): tblTareas
                ...
                ASUNCION DE ENTORNO: la base origen se asume restaurada
                como 'bdsApollo' (igual que los scripts 03/04).
   ===================================================================== */
BEGIN TRY
    ...
    DECLARE curTareas CURSOR LOCAL FAST_FORWARD FOR
        SELECT
            ...
            sol.IdSolicitud
        FROM bdsApollo.dbo.tblTareas t
        ...
        LEFT JOIN (SELECT DISTINCT idComplejidad, Complejidad FROM bdsApollo.dbo.tblComplejidad) co ON co.idComplejidad = t.Complejidad
    ...
    DECLARE curSub CURSOR LOCAL FAST_FORWARD FOR
        ...
        FROM bdsApollo.dbo.tblSubtareas s
        ORDER BY s.Id;
    ...
    SELECT
        ...
        N'',
        NULL
    FROM bdsApollo.dbo.tblHistorialEstatus h
